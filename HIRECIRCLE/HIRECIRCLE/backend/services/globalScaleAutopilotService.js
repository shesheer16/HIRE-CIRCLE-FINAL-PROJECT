...
const applyAutopilotFlags = ({ aiMode, infraMode, queueDepth, queueBackpressureActive }) => {
    const normalizedQueueDepth = Number(queueDepth || 0);
    const queuePressure = normalizedQueueDepth > 0
        && (queueBackpressureActive || normalizedQueueDepth >= INFRA_QUEUE_BACKPRESSURE_DEPTH);
