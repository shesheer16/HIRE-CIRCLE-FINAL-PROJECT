...
const installProcessSafeguards = ({ server }) => {
    ...
    process.on('unhandledRejection', (reason) => {
        // Known non-fatal error codes — log and continue, do NOT shut down.
        const NON_FATAL_CODES = ['DB_QUERY_TIMEOUT', 'DB_QUERY_LIMIT_EXCEEDED', 'DB_BULK_LIMIT_EXCEEDED'];
        if (reason && NON_FATAL_CODES.includes(reason.code)) {
            logger.warn({
                event: 'unhandled_rejection_non_fatal',
                code: reason.code,
