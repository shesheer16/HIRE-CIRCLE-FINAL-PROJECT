...
import {
    ...
    Modal, TextInput, KeyboardAvoidingView, Platform, Alert, Image, ActivityIndicator, Animated, Keyboard, Dimensions, RefreshControl
} from 'react-native';
...
const PROFILE_STUDIO_STAGE_ORDER = ['family', 'role', 'basics', 'fit', 'skills', 'photo'];
const PROFILE_STUDIO_STAGE_META = Object.freeze({
    family: {
        header: 'Choose work family',
        title: 'What kind of work are you looking for?',
        subtitle: 'Start with the family that fits your profile best.',
    },
    role: {
        header: 'Choose role',
        title: 'Pick your role title',
        subtitle: 'Search your role or choose one that matches the family.',
    },
    basics: {
        header: 'Basic details',
        title: 'Set your core details',
        subtitle: 'Location, language, experience, and pay should feel effortless.',
    },
    fit: {
        header: 'Work preferences',
        title: 'Set your work style',
        subtitle: 'Shift, joining window, travel, and fit signals help matching.',
    },
    skills: {
        header: 'Skills and proof',
        title: 'Show what you can do',
        subtitle: 'Add your strongest skills and any certificates you have.',
    },
    photo: {
        header: 'Profile photo',
        title: 'Finish your profile',
        subtitle: 'Add a photo and save your profile beautifully.',
    },
});
...
const resolveProfileStudioStage = (profile = {}, roleCategory = '') => {
    const normalizedCategory = String(roleCategory || inferRoleCategory(String(profile?.roleTitle || '').trim()) || '').trim();
    if (!normalizedCategory) return 'family';
    if (!String(profile?.roleTitle || '').trim()) return 'role';
    if (!isBasicsReadyForStudio(profile)) return 'basics';
    if (!isJobFitReadyForStudio(profile)) return 'fit';
    if (!isSkillsReadyForStudio(profile)) return 'skills';
    return 'photo';
};
...
export default function ProfilesScreen({ navigation }) {
    ...
    const hasRoleInfo = isRoleReadyForStudio(editingProfile, effectiveRoleCategory);
    const hasBasicsInfo = hasRoleInfo && isBasicsReadyForStudio(editingProfile);
    const hasFitInfo = hasBasicsInfo && isJobFitReadyForStudio(editingProfile);
    const canSaveStudioProfile = hasBasicsInfo && isSkillsReadyForStudio(editingProfile);
    const studioStageMeta = PROFILE_STUDIO_STAGE_META[studioStage] || PROFILE_STUDIO_STAGE_META.family;
    const roleFamilyVisual = getRoleCategoryVisual(effectiveRoleCategory);
    const isProfileFinalStage = studioStage === 'photo';
    const studioFooterLabel = isProfileFinalStage ? 'Save profile' : 'Continue';
    const studioFooterEnabled = isProfileFinalStage
        ? canSaveStudioProfile
        : studioStage === 'role'
            ? hasRoleInfo
            : studioStage === 'basics'
                ? hasBasicsInfo
    ...
    const fetchProfileData = useCallback(async ({ preservePreviousOnIncomplete = true } = {}) => {
        ...
        try {
            if (SCREENSHOT_MOCKS_ENABLED) {
                setProfiles(SCREENSHOT_PROFILES);
                setIsLoading(false);
                setErrorMsg('');
            ...
            const normalizedMessage = String(e?.message || '').trim().toLowerCase();
            ...
            if (isProfileRoleGateError(e)) {
                ...
                // Role gate: not a user-facing error — clear silently
                setErrorMsg('');
                return;
            }
            if (isTimedOut && preservePreviousOnIncomplete) {
                // Keep the profile tab usable if the network is slow; stale data or empty-state can still render.
                setErrorMsg('');
                return;
            }
            if (!preservePreviousOnIncomplete) {
            ...
            setErrorMsg(msg || 'Could not load profile. Check your connection and retry.');
        } finally {
            if (requestId === profileRequestIdRef.current) {
                setIsLoading(false);
            }
    ...
    const fetchPools = useCallback(async () => {
        ...
        try {
            ...
            const msg = String(
                e?.response?.data?.message
                || e?.message
                || 'Could not load talent pools. Check your connection and retry.'
            ).trim();
            setErrorMsg(msg || 'Could not load talent pools. Check your connection and retry.');
        } finally {
    ...
    const fetchPoolCandidates = useCallback(async (jobId, { preserveSelection = true } = {}) => {
        ...
        try {
            if (SCREENSHOT_MOCKS_ENABLED) {
                const candidates = Array.isArray(SCREENSHOT_TALENT_CANDIDATES[normalizedJobId])
                    ? SCREENSHOT_TALENT_CANDIDATES[normalizedJobId]
                    : [];
                setPoolProfiles(candidates);
                setSelectedCandidate((previous) => {
                    if (!preserveSelection || !previous) return null;
                    return candidates.find((candidate) => String(candidate.id) === String(previous.id)) || null;
                });
                setIsLoading(false);
                setErrorMsg('');
                return;
            ...
            const msg = String(
                e?.response?.data?.message
                || e?.message
                || 'Could not load candidates. Check your connection and retry.'
            ).trim();
            setErrorMsg(msg || 'Could not load candidates. Check your connection and retry.');
            setPoolProfiles([]);
        } finally {
    ...
    const openEdit = (prof) => {
        ...
        const nextRoleCategory = String(normalized.roleCategory || inferRoleCategory(normalizedRoleTitle) || '').trim();
        setSelectedRoleCategory(nextRoleCategory);
        setStudioStage(resolveProfileStudioStage(seededProfile, nextRoleCategory));
    ...
    const handleSelectStudioFamily = useCallback((categoryLabel) => {
        handleSelectRoleCategory(categoryLabel);
        setStudioStage('role');
    }, [handleSelectRoleCategory]);

    const handleContinueStudioStage = useCallback(() => {
        if (studioStage === 'role') {
            if (!hasRoleInfo) {
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(() => {});
                Alert.alert('Choose role', 'Pick your work family and role title to continue.');
                return;
            }
        ...
        if (studioStage === 'basics') {
            if (!hasBasicsInfo) {
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(() => {});
                Alert.alert('Complete basics', 'Add location, language, experience, and expected pay to continue.');
                return;
            }
            setStudioStage('fit');
            return;
        ...
        if (studioStage === 'skills') {
            if (!canSaveStudioProfile) {
                Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(() => {});
                Alert.alert('Add skills', 'Choose at least one skill to continue.');
                return;
            }
            setStudioStage('photo');
        }
    }, [canSaveStudioProfile, hasBasicsInfo, hasRoleInfo, studioStage]);
    ...
    const uploadAvatarUri = useCallback(async (uri, mimeType = 'image/jpeg') => {
        ...
        try {
            ...
            try {
                await client.post('/api/settings/avatar-url', { avatarUrl: secureUrl, role: 'worker' });
            } catch (syncErr) {
            ...
            console.warn('Cloudinary Avatar Sync Failed:', error);
            return safeUri; 
        }
    }, []);
    ...
    const handleSave = async () => {
        ...
        setStudioToast('Profile saved');
    };

    const profileExperienceDisplay = useMemo(() => {
        const years = Number(editingProfile?.experienceYears);
        if (!Number.isFinite(years)) return '';
        if (years <= 0) return 'Fresher';
        return `${years} years`;
    }, [editingProfile?.experienceYears]);
    const profileSalaryDisplay = useMemo(() => {
        const pay = Number(editingProfile?.expectedSalary || 0);
        if (!Number.isFinite(pay) || pay <= 0) return '';
        return `₹${pay.toLocaleString('en-IN')}`;
    }, [editingProfile?.expectedSalary]);
    const handleExperienceValueChange = useCallback((value) => {
        const raw = String(value || '').trim();
        const presetMatch = EXPERIENCE_CARD_OPTIONS.find((item) => normalizeToken(item.label) === normalizeToken(raw));
        if (presetMatch) {
            setIsCustomExperience(false);
            patchEditingProfile({ experienceYears: presetMatch.value });
            return;
        }
        const parsed = Number.parseInt(raw.replace(/[^\d]/g, ''), 10);
        setIsCustomExperience(true);
        patchEditingProfile({ experienceYears: Number.isFinite(parsed) ? Math.max(0, parsed) : null });
    }, [patchEditingProfile]);
    const handleSalaryValueChange = useCallback((value) => {
        const parsed = Number.parseInt(String(value || '').replace(/[^\d]/g, ''), 10);
        setIsCustomSalary(true);
        patchEditingProfile({ expectedSalary: Number.isFinite(parsed) ? Math.max(0, parsed) : null });
    }, [patchEditingProfile]);

    const renderStudioModalBody = () => {
        if (!editingProfile) return null;

        const StageIntro = () => (
            <View style={styles.stageIntro}>
                {studioStage !== 'family' ? (
                    <View style={styles.stageFamilyRow}>
                        <View style={[styles.stageFamilyPill, { backgroundColor: roleFamilyVisual.tint }]}>
                            <Text style={styles.stageFamilyEmoji}>{roleFamilyVisual.emoji}</Text>
                            <Text style={styles.stageFamilyText} numberOfLines={1}>{effectiveRoleCategory}</Text>
                        </View>
                        <TouchableOpacity
                            style={styles.stageChangeBtn}
                            onPress={() => setStudioStage('family')}
                            activeOpacity={0.88}
                        >
                            <Text style={styles.stageChangeText}>Change family</Text>
                        </TouchableOpacity>
                    </View>
                ) : null}
                <Text style={styles.stageIntroTitle}>{studioStageMeta.title}</Text>
                <Text style={styles.stageIntroSubtitle}>{studioStageMeta.subtitle}</Text>
            </View>
        );
        ...
        return (
            <>
                <ScrollView
                    showsVerticalScrollIndicator={false}
                    contentContainerStyle={styles.studioModalScroll}
                    keyboardShouldPersistTaps="always"
                ...
                >
                    ...
                    {studioStage === 'family' ? (
                        <View style={styles.studioFamilyStage}>
                            <View style={styles.studioFamilyGrid}>
                                {ROLE_CATEGORY_OPTIONS.map((item) => {
                                    const selected = String(effectiveRoleCategory || '').trim() === item.label;
                                    const visual = getRoleCategoryVisual(item.label);
                                    return (
                                        <TouchableOpacity
                                            key={item.label}
                                            style={[styles.studioFamilyCard, selected ? styles.studioFamilyCardActive : null]}
                                            onPress={() => handleSelectStudioFamily(item.label)}
                                            activeOpacity={0.9}
                                        >
                                            <View style={[styles.studioFamilyCardIcon, { backgroundColor: visual.tint }]}>
                                                <Text style={styles.studioFamilyCardEmoji}>{visual.emoji}</Text>
                                            </View>
                                            <Text style={[styles.studioFamilyCardTitle, selected ? styles.studioFamilyCardTitleActive : null]}>
                                                {item.label}
                                            </Text>
                                        </TouchableOpacity>
                                    );
                    ...
                    {studioStage === 'role' ? (
                        <>
                            <View style={styles.studioPanelCard}>
                                {String(editingProfile.roleTitle || '').trim() ? (
                                    <View style={styles.studioActionRow}>
                                        <StudioChip
                                            label={aiAssistLoading ? 'Filling' : 'AI fill'}
                                            selected={false}
                                ...
                                {String(formAssistMessage || '').trim() ? (
                                    <Text style={styles.modalInfoText}>{formAssistMessage}</Text>
                                ) : null}
                                <View style={styles.fieldGroup}>
                                    <TypeaheadInput
                                        ...
                                        onSelectSuggestion={(value) => handleSelectRoleTitle(value, { category: effectiveRoleCategory })}
                            ...
                            {roleSpotlightOptions.length > 0 ? (
                                <View style={styles.studioPanelCard}>
                                    <Text style={styles.studioSectionLabel}>Recommended roles</Text>
                                    <View style={styles.studioChipRow}>
                                        {roleSpotlightOptions.slice(0, 8).map((item) => (
                                            <StudioChip
                                                key={item}
                                                label={item}
                                                selected={normalizeToken(editingProfile.roleTitle) === normalizeToken(item)}
                                                onPress={() => handleSelectRoleTitle(item, { category: effectiveRoleCategory })}
                                            />
                                        ))}
                                    </View>
                                </View>
                        ...
                        <View style={styles.studioPanelCard}>
                            <View style={styles.fieldGroup}>
                                <FieldPicker
                                    ...
                                    onSelect={(value) => patchEditingProfile({ location: value })}
                                    title="District"
                                    hint="Pick your district"
                                />
                                {hasChosenLocation ? (
                                    <FieldPicker
                                        label="Local area"
                                        ...
                                        onSelect={(value) => patchEditingProfile({ panchayat: value })}
                                        title="Local area"
                                        hint="Add your area"
                                    />
                                ) : null}
                                <FieldPicker
                                    ...
                                    suggestions={languageTypeaheadOptions}
                            ...
                            <Text style={styles.studioSectionLabel}>Experience</Text>
                            <View style={styles.studioChipRow}>
                                {EXPERIENCE_CARD_OPTIONS.map((item) => (
                                    <StudioChip
                                        key={`experience-${item.label}`}
                                        label={item.label}
                                        selected={Number(editingProfile.experienceYears) === Number(item.value)}
                                        onPress={() => {
                                            setIsCustomExperience(false);
                                            patchEditingProfile({ experienceYears: item.value });
                                        }}
                                    />
                                ))}
                            </View>
                            <View style={styles.fieldGroup}>
                                <FieldPicker
                                    ...
                                    displayValue={profileExperienceDisplay}
                                    placeholder="Level"
                                    suggestions={EXPERIENCE_CARD_OPTIONS.map((item) => item.label)}
                                    onChangeText={handleExperienceValueChange}
                                    onSelect={handleExperienceValueChange}
                                    keyboardType="number-pad"
                                    title="Experience"
                                    hint="Select or type your level"
                                />
                            </View>
                            <Text style={styles.studioSectionLabel}>Expected pay</Text>
                            <View style={styles.studioChipRow}>
                                {compactSalaryOptions.map((value) => (
                                    <StudioChip
                                        key={`pay-${value}`}
                                        label={`₹${Number(value).toLocaleString('en-IN')}`}
                                        selected={Number(editingProfile.expectedSalary) === Number(value)}
                                        onPress={() => {
                            ...
                            <View style={styles.fieldGroup}>
                                <FieldPicker
                                    label="Expected pay"
                                    value={editingProfile.expectedSalary ? String(editingProfile.expectedSalary) : ''}
                                    displayValue={profileSalaryDisplay}
                                    placeholder="₹ —"
                                    suggestions={compactSalaryOptions.map((value) => String(value))}
                                    onChangeText={handleSalaryValueChange}
                                    onSelect={handleSalaryValueChange}
                                    keyboardType="number-pad"
                                    title="Expected pay"
                                    hint="Type the pay you expect"
                                    last
                                />
                            </View>
                        </View>
                    ...
                    {studioStage === 'fit' ? (
                        <View style={styles.studioPanelCard}>
                            <Text style={styles.studioSectionLabel}>Shift</Text>
                            <View style={styles.studioChipRow}>
                                {SHIFT_OPTION_META.map((item) => (
                                    <StudioChip
                                        key={`shift-${item.label}`}
                                        label={item.label}
                                        selected={String(editingProfile.preferredShift || 'Flexible') === item.label}
                                        onPress={() => patchEditingProfile({ preferredShift: item.label })}
                                    />
                                ))}
                            </View>
                            <Text style={styles.studioSectionLabel}>Joining</Text>
                            <View style={styles.studioChipRow}>
                                {AVAILABILITY_OPTIONS.map((item) => (
                                    <StudioChip
                                        key={`availability-${item.label}`}
                                        label={item.label}
                                        selected={Number(editingProfile.availabilityWindowDays || 0) === Number(item.value)}
                                        onPress={() => patchEditingProfile({ availabilityWindowDays: item.value })}
                                    />
                                ))}
                            </View>
                            <Text style={styles.studioSectionLabel}>Travel</Text>
                            <View style={styles.studioChipRow}>
                                {COMMUTE_OPTION_META.map((item) => (
                                    <StudioChip
                                        key={`travel-${item.value}`}
                                        label={`${item.value} km`}
                                        selected={Number(editingProfile.maxCommuteDistanceKm || 25) === Number(item.value)}
                                        onPress={() => patchEditingProfile({ maxCommuteDistanceKm: item.value })}
                                    />
                                ))}
                            </View>
                            <Text style={styles.studioSectionLabel}>Preferences</Text>
                            <View style={styles.studioChipRow}>
                                <StudioChip
                                    label="Open now"
                                    selected={editingProfile.isAvailable !== false}
                                ...
                                <StudioChip
                                    label="Relocate"
                                    selected={Boolean(editingProfile.openToRelocation)}
                                    onPress={() => setEditingProfile((prev) => (
                                ...
                                <StudioChip
                                    label="Night shift"
                                    selected={Boolean(editingProfile.openToNightShift)}
                                    onPress={() => setEditingProfile((prev) => (
                    ...
                    {studioStage === 'skills' ? (
                        <View style={styles.studioPanelCard}>
                            <View style={styles.editorCardHeader}>
                                <Text style={styles.studioSectionLabelTight}>Skills</Text>
                                ...
                                <View style={styles.studioChipRow}>
                                    {guidedSkillSuggestions.map((skill) => (
                                        <StudioChip
                                            key={skill}
                                            label={skill}
                                            selected={Array.isArray(editingProfile.skills) && editingProfile.skills.some((item) => normalizeToken(item) === normalizeToken(skill))}
                                            onPress={() => addSkillToken(skill)}
                                        />
                                    ))}
                                </View>
                            ) : null}
                            {(editingProfile.skills || []).length > 0 ? (
                                ...
                                <Text style={styles.modalInfoText}>Select at least one skill.</Text>
                            )}
                            <View style={styles.fieldGroup}>
                                <TypeaheadInput
                                    value={skillInput}
                                    onChangeText={setSkillInput}
                                    suggestions={skillTypeaheadOptions}
                            ...
                            <View style={styles.editorCardHeader}>
                                <Text style={styles.studioSectionLabelTight}>Certificates</Text>
                                <View style={styles.editorCountPill}>
                                ...
                                <View style={styles.studioChipRow}>
                                    {guidedLicenseSuggestions.map((license) => (
                                        <StudioChip
                                            key={license}
                                            label={license}
                                            selected={Array.isArray(editingProfile.licenses) && editingProfile.licenses.some((item) => normalizeToken(item) === normalizeToken(license))}
                                            onPress={() => {
                                                setEditingProfile((prev) => {
                                                    const existing = Array.isArray(prev?.licenses) ? prev.licenses : [];
                                                    if (existing.some((item) => normalizeToken(item) === normalizeToken(license))) return prev;
                            ...
                            {(editingProfile.licenses || []).length > 0 ? (
                                <View style={styles.skillsRow}>
                                    {(editingProfile.licenses || []).map((license, index) => (
                                        <StudioChip
                                            key={`${license}-${index}`}
                                            label={license}
                                            selected
                                ...
                                <Text style={styles.modalInfoText}>Certificates are optional.</Text>
                            )}
                            <View style={styles.fieldGroup}>
                                <TypeaheadInput
                                    value={licenseInput}
                                    onChangeText={setLicenseInput}
                                    suggestions={licenseTypeaheadOptions}
                                    onSelectSuggestion={(value) => {
                                        const token = String(value || '').trim();
                                        if (!token) return;
                                        setEditingProfile((prev) => {
                                            const existing = Array.isArray(prev?.licenses) ? prev.licenses : [];
                                            if (existing.some((item) => normalizeToken(item) === normalizeToken(token))) return prev;
                                            return { ...prev, licenses: [...existing, token] };
                                        });
                                        setLicenseInput('');
                                    }}
                                    placeholder="Add certificate"
                                    pickerMode
                                    pickerTitle="Add certificate"
                                />
                            </View>
                        </View>
                    ) : null}
                    ...
                    {studioStage === 'photo' ? (
                        <View style={styles.studioPanelCard}>
                            <Text style={styles.studioSectionLabel}>Profile photo</Text>
                            <View style={styles.photoCard}>
                                {editingProfile.avatar ? (
                                    <Image source={{ uri: editingProfile.avatar }} style={styles.photoCardPreview} />
                                ) : (
                                    <View style={styles.photoCardEmpty}>
                                        <Text style={styles.photoCardEmptyText}>Add photo</Text>
                                    </View>
                                )}
                                <TouchableOpacity
                                    style={styles.photoCardButton}
                                    onPress={handleAvatarPress}
                                    activeOpacity={0.86}
                                >
                                    <Text style={styles.photoCardButtonText}>
                                        {isSavingProfile && uploadingAvatar ? 'Syncing' : editingProfile.avatar ? 'Change photo' : 'Choose photo'}
                                    </Text>
                                </TouchableOpacity>
                            </View>

                            <View style={styles.sectionGap} />
                            <View style={styles.studioSummaryGrid}>
                                <View style={styles.studioSummaryCell}>
                                    <Text style={styles.studioSummaryKey}>Role</Text>
                                    <Text style={styles.studioSummaryVal}>{String(editingProfile.roleTitle || '').trim() || 'Pending'}</Text>
                                </View>
                                <View style={styles.studioSummaryCell}>
                                    <Text style={styles.studioSummaryKey}>Location</Text>
                                    <Text style={styles.studioSummaryVal}>{String(editingProfile.location || '').trim() || 'Pending'}</Text>
                                </View>
                                <View style={styles.studioSummaryCell}>
                                    <Text style={styles.studioSummaryKey}>Pay</Text>
                                    <Text style={styles.studioSummaryVal}>{profileSalaryDisplay || 'Pending'}</Text>
                                </View>
                                <View style={styles.studioSummaryCell}>
                                    <Text style={styles.studioSummaryKey}>Skills</Text>
                                    <Text style={styles.studioSummaryVal}>{`${Array.isArray(editingProfile.skills) ? editingProfile.skills.length : 0} added`}</Text>
                                </View>
                            </View>
                        </View>
                    ) : null}
                ...
                {studioStage !== 'family' ? (
                    <FormFooter
                        label={studioFooterLabel}
                        enabled={studioFooterEnabled}
                        loading={isProfileFinalStage ? isSavingProfile : false}
                        onPress={isProfileFinalStage ? handleSave : handleContinueStudioStage}
                    />
                ) : null}
            </>
        );
    ...
    const handleStudioBackPress = useCallback(() => {
        const stageIndex = PROFILE_STUDIO_STAGE_ORDER.indexOf(studioStage);
        if (stageIndex > 0) {
            setStudioStage(PROFILE_STUDIO_STAGE_ORDER[stageIndex - 1]);
            return;
        }
        closeEditModal();
    }, [closeEditModal, studioStage]);
                    ...
                    >
                        {errorMsg ? (
                            <View style={styles.inlineErrorBanner}>
                                <Text style={styles.inlineErrorBannerText}>{errorMsg}</Text>
                                <TouchableOpacity onPress={() => fetchPools()} activeOpacity={0.8}>
                                    <Text style={styles.inlineErrorRetryText}>Retry</Text>
                                </TouchableOpacity>
                            </View>
                        ) : null}
                        {pools.map(pool => (
        ...
        return (
            <View style={[styles.flex1, { backgroundColor: '#ffffff' }]}>
                <View style={[styles.employeeTopBar, { paddingTop: insets.top + 8 }]}>
                    ...
                    >
                        <Ionicons name="pie-chart-outline" size={22} color="#0f172a" />
                    </TouchableOpacity>
                </View>
                    ...
                    <ScrollView
                        ...
                        refreshControl={
                            <RefreshControl
                                ...
                                onRefresh={async () => {
                                    setProfileRefreshing(true);
                                    await fetchProfileData({ preservePreviousOnIncomplete: true });
                                    setProfileRefreshing(false);
                                }}
                                tintColor="#6f4ef6"
                                colors={['#6f4ef6']}
                    ...
                    >
                        <View style={styles.employeeOverviewCard}>
                            {errorMsg ? (
                                <View style={styles.inlineErrorBanner}>
                                    <Text style={styles.inlineErrorBannerText}>{errorMsg}</Text>
                                    <TouchableOpacity onPress={() => fetchProfileData()} activeOpacity={0.8}>
                                        <Text style={styles.inlineErrorRetryText}>Retry</Text>
                                    </TouchableOpacity>
                                </View>
                            ) : null}
                            <View style={styles.employeeOverviewTopRow}>
                                ...
                                <View style={styles.employeeOverviewCopy}>
                                    <View style={styles.employeeOverviewPill}>
                                        ...
                                        <Text style={styles.employeeOverviewPillText}>
                                            {String(
                            ...
                            {/* Metrics moved to dashboard modal */}
                    ...
                    <View style={styles.dashboardModalBackdrop}>
                        <View style={styles.dashboardModalSheet}>
                            <View style={styles.dashboardModalHeader}>
                                <Text style={styles.dashboardModalTitle}>Dashboard</Text>
                            ...
                            <View style={styles.dashboardMetricsGrid}>
                                {profileStats.map((stat) => (
                                    <View key={stat.label} style={styles.dashboardMetricBox}>
                                        <Text style={styles.dashboardMetricValue}>{stat.value}</Text>
                                        <Text style={styles.dashboardMetricLabel}>{stat.label}</Text>
                                    </View>
                                ))}
                            </View>
    ...
    return (
        <View style={styles.container}>
                ...
                >
                    <View style={styles.modalSheet}>
                        ...
                        <View style={styles.modalHeader}>
                            <TouchableOpacity onPress={handleStudioBackPress} style={styles.modalCloseBtn} activeOpacity={0.85}>
                                {studioStage === 'family' ? (
                                    <IconX size={20} color={C.white60} />
                                ) : (
                                    <Ionicons name="chevron-back" size={20} color={C.white60} />
                                )}
                            </TouchableOpacity>
                            <View style={styles.modalHeaderCopy}>
                                <Text style={styles.modalTitle}>{studioStageMeta.header}</Text>
                            </View>
                            {isProfileFinalStage ? (
                                <TouchableOpacity onPress={handleSave} style={styles.modalSaveLink} activeOpacity={0.85} disabled={!canSaveStudioProfile || isSavingProfile}>
                                    <Text style={[styles.modalSaveLinkText, !canSaveStudioProfile ? styles.modalSaveLinkTextDisabled : null]}>Save</Text>
                                </TouchableOpacity>
                            ) : (
                        ...
                        {renderStudioModalBody()}

                        {false && editingProfile && (
                            <ScrollView
                                showsVerticalScrollIndicator={false}
                                contentContainerStyle={[styles.modalScroll, { paddingBottom: 140 }]}
                                keyboardShouldPersistTaps="always"
                                keyboardDismissMode="on-drag"
                                automaticallyAdjustKeyboardInsets={Platform.OS === 'ios'}
                            >
                        ...
                        {false ? (
                            <View style={styles.modalStickyFooter}>
                                <Text style={styles.modalStickyFooterMeta}>{studioFooterMessage}</Text>
                                <Animated.View
                                    style={[
                                        styles.saveBtn,
                                        { transform: [{ scale: saveCtaScale }] },
                                        (!canSaveStudioProfile || isSavingProfile) && styles.saveBtnDisabled,
                                    ]}
                                >
                                    <TouchableOpacity
                                        style={styles.saveBtnSurface}
                                        onPress={handleSave}
                                        activeOpacity={0.9}
                                        disabled={!canSaveStudioProfile || isSavingProfile}
                                    >
            ...
            <Toast visible={Boolean(studioToast)} message={studioToast} onHide={() => setStudioToast('')} />
        </View>
    );
}
...
const styles = StyleSheet.create({
    ...
    studioFamilyCardActive: {
        borderColor: C.accent,
        backgroundColor: C.accentDim,
    },
    studioFamilyCardIcon: {
        width: 64,
        height: 64,
        borderRadius: 20,
        alignItems: 'center',
    ...
    studioFamilyCardEmoji: {
        fontSize: 30,
    },
    studioFamilyCardTitle: {
        fontSize: 16,
        lineHeight: 22,
        fontWeight: '700',
        color: C.text,
    },
    studioFamilyCardTitleActive: {
        color: C.accentDeep,
    },
    studioPanelCard: {
        marginHorizontal: 16,
