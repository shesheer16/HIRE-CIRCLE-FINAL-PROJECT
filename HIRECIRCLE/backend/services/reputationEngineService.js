...
const recalculateReputationProfile = async ({ userId, reason = 'manual' }) => {
    ...
    const [
        ...
        workerProfileId
            ? Application.countDocuments({
                worker: workerProfileId,
                status: { $in: ['offer_accepted', 'accepted'] },
            })
            : 0,
        workerProfileId
            ? Application.countDocuments({
                worker: workerProfileId,
                status: { $in: ['offer_declined', 'rejected', 'withdrawn'] },
            })
            : 0,
        Application.countDocuments({
            $or: [
            ...
            status: { $in: ['offer_declined', 'withdrawn'] },
        }),
    ]);
    ...
    const noShowCount = fraudFlags.filter((row) => String(row.flagType || '').toLowerCase().includes('no_show')).length;
    ...
    const offersAccepted = Number(offersAcceptedCount || 0);
    const offersRejected = Number(offersRejectedCount || 0);
    const cancellationRate = toPct(safeDiv(Number(cancellationEventsCount || 0), interactionVolume) * 100);
    const ghostingRate = toPct(safeDiv(Number(openRiskFlags || 0), interactionVolume) * 100);
    ...
    const profilePayload = {
        ...
        response_time_avg: Number(avgResponseHours.toFixed(4)),
        hires_completed: successfulHires,
