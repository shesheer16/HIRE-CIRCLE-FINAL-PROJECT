...
const normalizeObjectId = (value, fieldName) => {
    ...
    if (mongoose.isValidObjectId(rawString)) {
        return rawString;
    }

    if (typeof value === 'object') {
        const nestedId = value?._id ?? value?.$oid;
        if (nestedId) {
            return normalizeObjectId(nestedId, fieldName);
        }

        const rawBuffer = value?.buffer;
        if (rawBuffer && typeof rawBuffer === 'object') {
            const bytes = Array.isArray(rawBuffer) ? rawBuffer : Object.values(rawBuffer);
            if (bytes.length === 12) {
                return new mongoose.Types.ObjectId(Buffer.from(bytes)).toString();
            }
        }
    }
...
const assertJobOwnership = async ({ employerId, jobId }) => {
    const normalizedJobId = normalizeObjectId(jobId, 'jobId');
...
const fundEscrow = async ({
    ...
    await assertJobOwnership({ employerId, jobId: normalizedJobId });
